          ...
          ElevatedButton(
            ...
            onPressed: () => {
              Provider.of<SessionRepository>(context, listen: false)
                  .logOut()
                  .then((_) => Navigator.pushNamedAndRemoveUntil(context, '/', (_) => false))
            },
          )
        ],
      ),
      body: CountriesListWidget(Provider.of<CountriesRepository>(context, listen: false)),
    );
  }
}
...
class CountriesListWidget extends StatefulWidget {
  ...
  const CountriesListWidget(this._repository);

  @override
  _CountriesListWidgetState createState() => _CountriesListWidgetState(_repository);
}
